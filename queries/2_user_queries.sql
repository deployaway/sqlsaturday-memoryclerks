...
-- takes 70-90 seconds
SELECT TOP 10 c.UserId FROM dbo.Comments AS c WHERE YEAR(c.CreationDate) IN (2016) GROUP BY c.UserId HAVING COUNT(c.PostId) > 3
...
FROM
    dbo.Comments AS c
WHERE
    YEAR(c.CreationDate) IN (2016,2017)
GROUP BY
    c.UserId
HAVING
